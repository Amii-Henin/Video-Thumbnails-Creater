...
def main():
    ...
    # print (alldirs)
    save_log('delete_video_thumb_log.json','a+',now + ':{\n')
    for path in alldirs:
        delete_thumb(path)
        # time.sleep(200)
    save_log('delete_video_thumb_log.json','a+','}\n')
...
def delete_thumb(path):        #开始程序
    ...
    for x in range(len(files)):
        ...
        if os.path.exists(path + '\\' + nfiles[x] + '_thumb.jpg'):      #判断是否已有缩略图
            os.remove(pic_name)
            save_log('delete_video_thumb_log.json','a+','    \"' + pic_name + '\",\n')
            print ('删除',pic_name)
